...
public class Main_T {
	...
	static int size; // zero의 size => 0 으로 미완성인 좌표의 수
	...
	public static void main(String[] args) throws Exception{
		...
		for (int i = 0; i < 9; i++) {
			...
			for (int j = 0; j < 9; j++) {
				int n = line[j] - '0';
				map[i][j] = n;
				if( n == 0 ) zero.add(new Node(i, j)); // 0 인 미완성 좌표를 zero에 담는다.
			}
		}
	...
	static void dfs(int idx) {
		...
		// 현재 0인 (idx) y, x좌표에 위에서 사용되지 않은 숫자( visit에 없는 숫자를 하나씩 넣어본다.)
		// 1, 4, 6, 7이 사용되었으면 2, 3, 5, 8, 9 숫자를 현재 y,x 좌표에 각각 채워보고 다시 다음 idx + 1에 채우기 위해 dfs()이어 간다.
		for (int i = 1; i <= 9; i++) {
			if( visit[i] ) continue;
			map[y][x] = i;
			dfs( idx + 1 );
